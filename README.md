...
| `PI_CURSOR_CACHE_DIR`                      | Where the model catalog and refresh back-off are cached (default: `$XDG_CACHE_HOME/pi/cursor`, normally `~/.cache/pi/cursor`). Delete only its `models.json` to force a full rediscovery.                                                                                                                                                                                                                                 |
...
synchronously from `$XDG_CACHE_HOME/pi/cursor/models.json` (or `$PI_CURSOR_CACHE_DIR/models.json`), falling back
...
- **Model list looks stale:** It is the last successfully discovered catalog. Open `/model` to trigger a background refresh, or delete `~/.cache/pi/cursor/models.json` (or `$PI_CURSOR_CACHE_DIR/models.json`) to force full rediscovery.
